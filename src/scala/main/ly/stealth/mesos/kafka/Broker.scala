...
class Broker(val id: Int = 0) {
  ...
  // broker has been modified while being in non stopped state,
  // once stopped or before task launch becomes false
  ...
  def matches(
    offer: Offer,
    now: Date = new Date(),
    otherAttributes: Broker.OtherAttributes = Broker.NoAttributes): OfferResult =
  {
    // check resources
    ...
    // check volume
    // Note: this MUST be the last decline check in order for volume auto-creation to work
    if (volumes.nonEmpty) {
      val unmatchedVolumes = volumes.map(_.volumeId).toSet --
        reservation.volumes.map(_.getDisk.getPersistence.getId)
      if (unmatchedVolumes.nonEmpty) {
        return OfferResult.DeclineMissingVolumes(offer, this)
      }
    }
  ...
  def getReservation(offer: Offer): Broker.Reservation = {
    ...
    val reservedVolumes = mutable.Buffer[Resource]()

    val volumeIds = volumes.map(_.volumeId).toSet
    ...
    for (resource <- offer.getResourcesList) {
      if (resource.getRole == "*") {
        ...
        if (resource.hasDisk
          && resource.getDisk.hasPersistence
          && resource.getDisk.getPersistence.hasId
          && volumeIds.contains(resource.getDisk.getPersistence.getId))
        {
          reservedVolumes.append(resource)
        }
      }
    }
...
object Broker {
  ...
  object Volume {
    def unapply(arg: Volume): Option[(String)] = Some(arg.volumeId)
  }

  case class StaticVolume(volumeId: String) extends Volume

  object DynamicVolumeState extends Enumeration {
    type DynamicVolumeState = Value
    val AwaitingOffer = Value // The volume is waiting for an offer to match it
    val Creating = Value // The volume has matched an offer, and an Accept() message has been sent
    val Ready = Value // The volume is ready to be used, we've seen an offer with it

    val Suspect = Value // There hasn't been an offer with this volume for X amount of time, and it could be destroyed.
  }
  import DynamicVolumeState._

  // Sorry for the jackson leaking here, but it seems like mixins dont work well with deserializing
  // enums
  private[this] class DynamicVolumeStateType extends TypeReference[DynamicVolumeState.type]
  case class DynamicVolume(
    sizeMb: Long,
    containerPath: String,
    volumeId: String,
    @JsonScalaEnumeration(value = classOf[DynamicVolumeStateType])
    state: DynamicVolumeState = AwaitingOffer,
    allowedSources: Set[Source.Type] = Set()
  ) extends Volume

  object PendingVolume {
    def unapply(arg: Volume): Option[DynamicVolume] =
      arg match {
        case v@DynamicVolume(_, _, _, DynamicVolumeState.AwaitingOffer, _) => Some(v)
        case v@DynamicVolume(_, _, _, DynamicVolumeState.Suspect, _) => Some(v)
        case _ => None
      }
  }

  object CreatedVolume {
    def unapply(arg: Volume): Option[Volume] =
      arg match {
        case _: StaticVolume => Some(arg)
        case v: DynamicVolume
          if v.state == DynamicVolumeState.Creating || v.state == DynamicVolumeState.Ready => Some(arg)
        case _ => None
      }
  }
  ...
  object Endpoint {
    def apply(s: String): Endpoint = valueOf(s)
    def valueOf(s: String): Endpoint = {
      val idx = s.indexOf(":")
      if (idx == -1) throw new IllegalArgumentException(s)

      Endpoint(s.substring(0, idx), Integer.parseInt(s.substring(idx + 1)))
    }
  }
