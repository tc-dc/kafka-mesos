          ...
          broker.executionOptions = broker.executionOptions.copy(
            container =
              broker.executionOptions.container
                .map(c => c.copy(name = containerImage))
                .orElse(
                  Some(Container(ctype = ContainerType.Docker, name = containerImage))))
        }
        if (containerType.isDefined && broker.executionOptions.container.isDefined) {
          broker.executionOptions = broker.executionOptions.copy(
            container = broker.executionOptions.container.map(_.copy(ctype = containerType.get))
          )
        }
